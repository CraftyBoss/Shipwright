...
void EnReeba_Init(Actor* thisx, PlayState* play) {
    ...
    // if we're in battle hall mode, set found surface type to Sand to prevent instant actor kill
    if (IS_BATTLE_HALL) {
        surfaceType = 4;
    }
...
void EnReeba_Move(EnReeba* this, PlayState* play) {
    ...
    // if we're in battle hall mode, set found surface type to Sand to prevent immediate sinking
    if (IS_BATTLE_HALL) {
        surfaceType = 4;
    }
...
void EnReeba_SetupSink(EnReeba* this, PlayState* play) {
    ...
    {
        // prevents sinking from overlapping death, which creates an invincible leever in battle hall.
...
void EnReeba_Sink(EnReeba* this, PlayState* play) {
    ...
    if ((this->unk_284 + 10.0f) <= this->actor.shape.yOffset) {
        ...
        // go back to surfacing when fully sunk in battle hall, so the enemy can be killed by the player.
        if (IS_BATTLE_HALL) {
            this->actionfunc = EnReeba_SetupSurface;
        } else {
            Actor_Kill(&this->actor);
...
void EnReeba_StunDie(EnReeba* this, PlayState* play) {
    ...
    if (this->waitTimer != 0) {
        if ((this->unk_27E == 2) && ((this->waitTimer & 0xF) == 0)) {
            pos.x = this->actor.world.pos.x + Rand_CenteredFloat(20.0f);
            pos.y = this->actor.world.pos.y + Rand_CenteredFloat(20.0f);
            pos.z = this->actor.world.pos.z + Rand_CenteredFloat(20.0f);
    ...
    if (this->waitTimer != 0) {
        ...
        if (this->scale < 0.01f) {
            ...
            if (this->actor.parent != NULL) {
                ...
                Actor_Kill(&this->actor);
                GameInteractor_ExecuteOnEnemyDefeat(&this->actor);
            } else if(IS_BATTLE_HALL) { // kill individual leever if in battle hall mode
                Actor_Kill(&this->actor);
                GameInteractor_ExecuteOnEnemyDefeat(&this->actor);
            }
        }
